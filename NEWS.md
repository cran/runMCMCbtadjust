# runMCMCbtadjust 1.1.3

Mainly technical bug fixes:
- due to CRAN constraints, I had to put parallelizeInitExpr in control.MCMC to NULL. The user now has to specify the expression himself. This implied rather numerous changes, including in vignettes.
- changed the declaration of Author in DESCRIPTION file
- due to incompatibilities with the 1.4.0 version of Nimble, changed the constraints on Nimble version in DESCRIPTION file
...
- minor clarity changes in help (of control$conv.thorough.check, control$remove.fixedchains)
...
- Added two vignettes to give explanations on two specific points: changing samplers with Nimble and doing extra-calculations with Nimble
...
- Introduction of control$conv.thorough.check and associated code in sections 1.4 & 2.4.1
...
- Reframed the proposed number of iterations to run in 2.1 in case  ! Converged & ! previously.converged (to be close to case where previously converged in case the diagnostic is close the target, using a sigmoid function) & Heidelberger replaced by Heidelberger. Added associated component ip.nc to control
...
- Removed code: require(coda) through adding more calls to coda::as.mcmc.list to link with coda library sooner - for functions such as summary, as.matrix and as.array
...
- To correct problems with calculus of nburnin in case index.conv==1, replaced throughout: nburnin.min0+sum(numIter.samplesList[1:ifelse(converged,index.conv-1,size.samplesList)]) by: nburnin.min0+ifelse(index.conv==1&converged,0,sum(numIter.samplesList[1:ifelse(converged,index.conv-1,size.samplesList)]))
...
- Removed components of the attributes of the output: R.version and package.versions, replacing it by sessionInfo that contains these info (+ extra info)
